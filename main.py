...
if not df_atual.empty:
    ...
    # --- ABAS 1 E 2 (Mantidas Conforme Seu Código) ---
    if aba == "📝 EDIÇÃO E QUADRO":
        # ... (Seu código de edição, cadastro e exclusão igual ao que enviou)
        ...
        with st.form("form_edit_final"):
            ...
            st_atual = calcular_status_tag(v_ini, v_fim, v_mont); st.info(f"Status: {st_atual}")
        ...
        with col_del:
            with st.expander("🗑️ DELETAR TAG"):
                tag_del = st.selectbox("TAG para DELETAR:", [""] + sorted(df_atual['TAG'].unique().tolist()))
                if tag_del and st.button("🔴 CONFIRMAR EXCLUSÃO"):
                    cell = ws_atual.find(tag_del, in_column=1)
                    if cell: ws_atual.delete_rows(cell.row); st.rerun()

    elif aba == "📊 CURVA S":
        # ... (Seu código de Curva S igual ao que enviou)
        st.subheader(f"📊 Curva S - {disc}")
        # (Lógica do Plotly que você enviou...)

    # --- ABA 3: RELATÓRIOS (RESTAURADA CONFORME SEU CÓDIGO ORIGINAL) ---
    elif aba == "📋 RELATÓRIOS":
    ...
    # --- ABA 4 (Mantida Conforme Seu Código) ---
    elif aba == "📤 EXPORTAÇÃO E IMPORTAÇÕES":
        # ... (Seu código de exportação igual ao que enviou)
        st.subheader(f"📤 Exportações - {disc}")
