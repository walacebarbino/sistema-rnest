...
# --- CSS PARA PADRONIZAÇÃO E CENTRALIZAÇÃO ---
st.markdown("""
    <style>
    ...
    /* Centralização da Logo na Sidebar e Login */
    [data-testid="stSidebar"] [data-testid="stImage"], .stImage {
        display: flex;
...
if not df_atual.empty:
    ...
    # --- ABA 3: RELATÓRIOS ---
        ...
        # Tratamento do Seletor de Semanas
        semanas_disponiveis = sorted([s for s in df_atual['SEMANA OBRA'].unique() if s], reverse=True)
        if semanas_disponiveis:
            ...
            st.dataframe(df_semana[['TAG', 'DESCRIÇÃO', 'DATA MONT', 'ÁREA', 'OBS']], use_container_width=True, hide_index=True)
            
            buf_r = BytesIO()
            df_semana.to_excel(buf_r, index=False)
            st.download_button(f"📥 EXPORTAR SEMANA {semana_sel}", buf_r.getvalue(), f"Avanco_Semana_{semana_sel}.xlsx")
        else:
            st.info("Nenhuma semana registrada para avanço.")
                ...
                if st.button("🚀 IMPORTAR E LIMPAR DADOS", use_container_width=True):
                    try:
                        ...
                        if sucesso > 0:
                            ...
                            st.success(f"✅ Sucesso! {sucesso} TAGs sincronizadas.")
                            st.rerun()
                    except Exception as e:
                        st.error(f"❌ Erro: {e}")
